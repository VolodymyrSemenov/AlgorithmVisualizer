...
class VisualizationState(WindowState):
    ...
    def what_is_clicked(self, mouse_position):
        ...
        if mouse_y < self.av.h - 50:
            return math.floor(mouse_x / BLOCK_SIZE), math.floor(mouse_y / BLOCK_SIZE)
        else:
    ...
    def enter(self):
        ...
        if self.av.selectedAlgo == Algo.BREADTH_FIRST_SEARCH:
            self.av.depths, self.av.completed_path = breadth_first_search(drawing_edges, self.av.start, self.av.end, self.av.fullExplore)
        ...
        if self.av.selectedAlgo == Algo.A_STAR_SEARCH:
            self.av.depths, self.av.completed_path, self.av.ordered_visited_nodes = a_star_search(drawing_edges, self.av.start, self.av.end, self.av.fullExplore, True)
            self.av.maxDepth = max(x for x in self.av.depths.values())
            self.av.rainbow_speed_multiplier = math.floor(300 / self.av.maxDepth)
            if self.av.rainbow_speed_multiplier < 0.0001:
                self.av.rainbow_speed_multiplier = 1
        ...
        if self.av.selectedAlgo == Algo.A_STAR_SEARCH_FAST:
            self.av.depths, self.av.completed_path, self.av.ordered_visited_nodes = a_star_search(drawing_edges, self.av.start, self.av.end, self.av.fullExplore, False)
            self.av.maxDepth = max(x for x in self.av.depths.values())
            self.av.rainbow_speed_multiplier = math.floor(300 / self.av.maxDepth)
            self.av.switch_astar_state()
